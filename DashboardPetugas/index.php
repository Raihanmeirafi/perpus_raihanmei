...
$query = mysqli_query($connection, "SELECT * FROM peminjaman WHERE status = 'Tunggu konf'");
...
<img src="../assets/library Book Sale.png" alt="logo" width="50px">
      ...
      <div class="p-3 mb-2 border bg-secondary">
        <tr>
          <th>
      <div class="bg-warning alert alert-success" role="alert">Selamat datang petugas - <span class="fw-bold text-capitalize"><?php echo $_SESSION['admin']['nama_admin']; ?></span> di Dashboard Perpustakaan</div>
      </th>  
    </tr>
     ...
     <thead class="text-center">
      ...
      <?php foreach ($peminjaman as $data): ?>
                <tr>
                    ...
                    <td><?= $data["status"]; ?></td>
                    <td><a href="konfirmasi_peminjaman.php?id=<?= $data['id_peminjaman']; ?>" class="btn btn-success">Konfirmasi</a><a href="deleteMember.php?id=<?= $item["nisn"]; ?>" class="btn btn-danger" onclick="return confirm('Yakin ingin menghapus data member ?');"><i class="fa-solid fa-trash">Hapus  </i></a></td>
                </tr>
                <?php endforeach; ?>
    </table>
    </div>
